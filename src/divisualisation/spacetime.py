"""Spacetime renderer: fold time into the z-axis for a 3D animation.

This is the original ``Divisualisation`` use case, where the whole setup is
contained in a class because it drives a scripted animation render. For the
interactive/modular use case (adding error layers to an existing viewer) see
``divisualisation.errors.add_edge_error_tracks``.
"""
...
class Divisualisation:
    ...
    def visualize_gt(
        ...
        gt_tracks, _, properties = graph_to_napari_tracks(
            gt_graph,
        ...
        assert gt_tracks.shape[1] == 5
        gt_tracks[:, -3] = gt_tracks[:, -3] + self.time_scale * gt_tracks[:, -4]
        ...
        if pred_graph is not None:
            ...
            pred_properties["pred"] = np.ones_like(pred_properties[time_attr]) * 0.5
            ...
            pred_tracks_layer = v.add_tracks(
                ...
                blending="translucent_no_depth",
                colormaps_dict={
                    "pred": vispy_or_mpl_colormap("Wistia"),
                },
                tail_width=self.tracks_width,
                tail_length=1000,
                opacity=1.0,
            ...
            v.dims.events.point.connect(update_pred_state)
